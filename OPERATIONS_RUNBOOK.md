...
2. **SableWeb container shared the same compose project** — three fixes landed in the same override (next_cache named volume, `HOSTNAME=0.0.0.0`, healthcheck `127.0.0.1` instead of `localhost`). See the comment block explaining each in SableWeb's **tracked `docker-compose.prod-override.yml`**. ⚠️ **Changed 2026-07-26:** the box's `/opt/sable-web/docker-compose.override.yml` is now a **symlink** to that tracked file, and the `sable-roles` / `sable-roles-tig` service definitions live there. **Never edit the override on the box** — it dirties a tracked file and the next `git pull --ff-only` aborts. To change bot config: edit SableWeb's `docker-compose.prod-override.yml` → push to main → `sudo -u sable git pull --ff-only origin main` on the box → `docker compose up -d sable-roles` (a pull alone restarts nothing). Bot **secrets** still live in the box's gitignored `/opt/sable-web/.env` as `${VAR}` refs — never inline a token in the tracked file.
